...
export function Timeline({ steps, current, maxSelectableIndex = steps.length - 1, onSelect }: TimelineProps) {
  return (
    <div className="rounded-3xl border border-slate-700/35 bg-slate-950/42 p-4 shadow-[0_18px_70px_rgba(0,0,0,0.28)] backdrop-blur-xl">
      <div className="mb-3 flex items-center justify-between">
        <span className="font-mono-data text-xs uppercase tracking-[0.3em] text-slate-400">流程时间线</span>
        <span className="text-xs text-slate-500">点击步骤可查看对应通信过程</span>
      </div>
